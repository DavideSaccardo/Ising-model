...
int main(int argc, char* argv[])
{
    ...
    double initialTemp=2.1; //temperature at which we start our calculations
    double finalTemp=2.5; //temperature at which we end our calculations
    int numberOfSpins=40; // number of spin of the lattice considered, the dimension is thus L=numberOfSpinsXnumberOfSpins
...
void MCcomputation(long int numberOfMCcycles, int numberOfSpins, double Temperature, double* local_expectation_value, int RankProcess, int NProcesses, int flag){
    ...
    initializeLattice(numberOfSpins, matrixOfSpins, energy, magnetic_moment, RankProcess, flag);
    ...
    for(long int cycle=1;cycle<=numberOfMCcycles;cycle++){
        ...
        if(cycle>5e4){
            ...
            local_expectation_value[3]+=magnetic_moment*magnetic_moment;
            local_expectation_value[4]+=fabs(magnetic_moment);
        }
    }
    for (int i=0; i<numberOfSpins;i++){
...
void initializeLattice(int numberOfSpins, double** matrixOfSpins, double &energy, double &magnetic_momentum, int RankProcess, int Flag){
